...
- 설정창(게임 속도, 인식 감도, 카메라 미리보기)
- 조작은 게임과 직접 연동해 **입력 지연 최소화**
...
- **키 매핑 설정 없음** — 이 게임에 좌/우/점프를 직접 연동(지연 최소화). 다른 게임 재사용 대비 불필요.
- WebSocket 브릿지, 리더보드, 무인 자동 리셋
- 게임 3D 지오메트리/모델 변경은 **초기 범위 밖(차후 수정 가능)**. 초기엔 텍스처 교체·속도 조정만.

## 3. 아키텍처

기존 게임 위에 부스 셸(shell) 레이어를 얹는다. **게임 내부 로직은 수정하지 않고**,
조작은 매 프레임 판정 즉시 `Mousetrap.trigger('left'|'right'|'up')`를 **직접 동기 호출**해
입력 지연을 최소화한다. (게임은 same-origin에서 로드하므로 trigger 접근 가능. trigger가
안 먹히면 게임 함수 직접 호출로 폴백)
...
│  ├─ adapter.js            # 추상 동작 → 게임 입력(Mousetrap.trigger 직접 호출, 최소 지연)
...
> 키 매핑 설정은 두지 않는다(비목표). 조작은 좌/우/점프를 게임에 직접 연동.
...
- **실행**: 정적 서버를 `localhost`로 띄우고 Chrome 접속 후 전체화면(F11/키오스크).
  ...
  - 서버 스크립트는 동봉하지 않고 **실행 안내(README)만 제공**: `python -m http.server` 등
    이미 있는 도구로 폴더에서 한 줄 실행.
- **배포**: 폴더 파일 전달 또는 `git clone` 방식. 받은 쪽이 폴더에서 위 서버 한 줄로 실행.
  Mac에서 개발·테스트 → 윈도우 PC에서 동일하게 clone/복사 후 실행.
...
- **배포**: 파일 전달 또는 `git clone`. 서버는 동봉 안 하고 **실행 안내만** 제공.
- **키 매핑**: 두지 않음. 좌/우/점프를 게임에 직접 연동, 입력 지연 최소화.
- **게임 원본**: 클론 그대로 사용. 모델링/외형 변경은 차후 과제.

### 남은 구현 세부(계획 단계에서 확정)
- MediaPipe Tasks(web)의 로컬 wasm/모델 경로 구성 방식.
- 게임을 same-origin iframe으로 감쌀지, 동일 페이지에 포함할지(네임스페이스 vs 접근성).
