...
const calibEl = $("calib"),
  ...
  calibRing = document.querySelector("#calib .prog");
const fxEl = $("fx"),
  fxFlash = $("fx-flash"),
  fxVig = $("fx-vig");
...
// (씬 값 = 2초) 동안 _canVulnerable 이 false 이므로 그 길이에 맞춘다.
...
function renderScore() {
  ...
  if (total > bestScore) {
    bestScore = total;
    try {
      storage.setItem(BEST_KEY, String(bestScore));
    } catch {
      /* 시크릿 모드 */
    }
  }
...
unityContainer.addEventListener("animationend", () =>
  unityContainer.classList.remove("shake"),
);
...
// 같은 파일명으로 WebGL을 재빌드해도 브라우저/CDN이 이전 대용량 파일을
// 재사용하지 않도록 빌드마다 이 값으로 URL을 구분한다.
const UNITY_BUILD_REV = "20260828-023226";
const unityAssetUrl = (name) =>
  `${UNITY_BUILD}/${name}?v=${UNITY_BUILD_REV}`;
const UNITY_CONFIG = {
  dataUrl: unityAssetUrl("game-unity.data.gz"),
  frameworkUrl: unityAssetUrl("game-unity.framework.js.gz"),
  codeUrl: unityAssetUrl("game-unity.wasm.gz"),
...
function injectUnityLoader() {
  return new Promise((resolve, reject) => {
    if (unityLoaderInjected) {
      resolve();
      return;
    }
    const s = document.createElement("script");
    s.src = unityAssetUrl("game-unity.loader.js");
    s.onload = () => {
      unityLoaderInjected = true;
      resolve();
    };
    s.onerror = () =>
      reject(new Error("Unity 로더(game-unity.loader.js) 로드 실패"));
...
function ensureUnity() {
  ...
  unityReady = (async () => {
    ...
    try {
      ...
      hintEl.textContent =
        "게임 로드 실패: " + (e && e.message ? e.message : e);
...
async function ensureCamera() {
  ...
  try {
    ...
    hintEl.textContent =
      "카메라 권한이 필요합니다 — 허용 후 새로고침해주세요. (Esc: 메뉴)";
    return false;
  }
  video = v;
  return true;
...
function stopFrameLoop() {
  if (vfcHandle && video && video.cancelVideoFrameCallback)
...
function tick(t = performance.now()) {
  ...
  lastTickAt = t;
  if (phase !== "calibrate" && phase !== "play") return;
  if (!usingVfc && t - lastDetectAt >= DETECT_FALLBACK_MS) {
    lastDetectAt = t;
    onCameraFrame(t);
  }
  if (phase === "play") {
    updateSpeed();
    accumulateDistance(dt);
  }
  updateHitFx(t, dt);
}
...
function toMenu() {
  ...
  inputGate = false;
  sendSpeed(unity, 0);
  lastSentSpeed = 0; // 메뉴에선 게임을 세워 둔다
...
function attachSettingsListeners() {
  ...
  corner.addEventListener("change", () => {
    settings.previewCorner = corner.value;
    saveSettings(storage, settings);
  });
...
window.addEventListener("keydown", (e) => {
  if (e.key === "c" || e.key === "C") {
    if (phase === "play" || phase === "calibrate") recalibrateNow();
  }
